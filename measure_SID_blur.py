...
def metrics(im_dir, label_dir, use_GT_mean, loss_fn, device):
    avg_psnr = 0
    avg_ssim = 0
    avg_lpips = 0
    n = 0
...
if __name__ == '__main__':
    ...
    mea_parser.add_argument('--cpu', action='store_true', help='Run LPIPS on CPU (slower)')
    ...
    if mea.cpu:
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("No GPU found; pass --cpu or use a CUDA environment.")
    ...
    if mea.Blur:
        for index in range(1, 257):
            ...
            test_dir = "data_dir/output/LOL_Blur/"
            im_dir = test_dir + fill_index + "/*.png"
            label_dir = 'data_dir/LOL_blur/test/high_sharp_scaled/' + fill_index + "/"
            if os.path.exists(test_dir + fill_index):
                i_psnr, i_ssim, i_lpips, i_n = metrics(im_dir, label_dir, mea.use_GT_mean, loss_fn, device)
        ...
        for index in range(1, 257):
            ...
            test_dir = "data_dir/output/SID/"
            im_dir = test_dir + fill_index + "/*.png"
            label_dir = 'data_dir/Sony_total_dark/test/long/' + fill_index + "/"
            if os.path.exists(test_dir + fill_index):
                i_psnr, i_ssim, i_lpips, i_n = metrics(im_dir, label_dir, mea.use_GT_mean, loss_fn, device)
                print("===> Finish " + fill_index + " folder")
                print("===> Avg.PSNR: {:.4f} dB ".format(i_psnr / i_n))
