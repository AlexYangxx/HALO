...
def _triton_tensor_device() -> torch.device:
    """Triton kernels require CUDA tensors; device follows availability."""
...
def weighted_estimate_se3_triton(source_points, target_points, weights):
    ...
    source_points = torch.from_numpy(source_points).to(_triton_tensor_device()).float()
    target_points = torch.from_numpy(target_points).to(_triton_tensor_device()).float()
    weights = torch.from_numpy(weights).to(_triton_tensor_device()).float()
...
def weighted_estimate_sim3_numba_triton(
    ...
    H_torch = torch.from_numpy(H).to(_triton_tensor_device()).float()
...
def robust_weighted_estimate_sim3_triton(
    ...
    dev = _triton_tensor_device()
    src_torch = torch.from_numpy(src).to(dev).float()
    tgt_torch = torch.from_numpy(tgt).to(dev).float()
    init_weights_torch = torch.from_numpy(init_weights).to(dev).float()

    s, R, t = weighted_estimate_sim3_numba_triton(
        src, tgt, init_weights, align_method=align_method
    )

    R_torch = torch.from_numpy(R).to(dev).float()
    t_torch = torch.from_numpy(t).to(dev).float()
    s_torch = torch.tensor(s, device=dev, dtype=torch.float32)
...
def warmup_triton():
    ...
    wdev = _triton_tensor_device()
    src_torch = torch.from_numpy(src).to(wdev).float()
    tgt_torch = torch.from_numpy(tgt).to(wdev).float()
    weights_torch = torch.from_numpy(weights).to(wdev).float()
    ...
    R_torch = torch.from_numpy(R).to(wdev).float()
    t_torch = torch.from_numpy(t).to(wdev).float()
    s_torch = torch.tensor(s, device=wdev, dtype=torch.float32)
    ...
    try:
        residuals = torch.abs(torch.randn(n_points, device=wdev, dtype=torch.float32))
