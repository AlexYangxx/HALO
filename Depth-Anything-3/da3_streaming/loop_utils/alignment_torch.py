...
def _align_device() -> torch.device:
    return torch.device("cuda" if torch.cuda.is_available() else "cpu")


def weighted_estimate_se3_torch(source_points, target_points, weights):
    dev = _align_device()
    source_points = torch.from_numpy(source_points).to(dev).float()
    target_points = torch.from_numpy(target_points).to(dev).float()
...
def weighted_estimate_sim3_numba_torch(source_points, target_points, weights, align_method="sim3"):
    ...
    H_torch = torch.from_numpy(H).to(_align_device()).float()
...
def huber_loss_torch(r, delta):
    ...
    delta_torch = torch.tensor(delta, device=dev, dtype=torch.float32)
...
def compute_residuals_torch(tgt, transformed):
    ...
    transformed_torch = torch.from_numpy(transformed).to(dev).float()
...
def compute_huber_weights_torch(residuals, delta):
    dev = _align_device()
    residuals_torch = torch.from_numpy(residuals).to(dev).float()
    delta_torch = torch.tensor(delta, device=dev, dtype=torch.float32)
