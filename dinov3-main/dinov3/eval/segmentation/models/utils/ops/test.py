...
_TEST_DEV = torch.device("cuda" if torch.cuda.is_available() else "cpu")

N, M, D = 1, 2, 2
Lq, L, P = 2, 2, 2
shapes = torch.as_tensor([(6, 4), (3, 2)], dtype=torch.long, device=_TEST_DEV)
...
def check_forward_equal_with_pytorch_double():
    value = torch.rand(N, S, M, D, device=_TEST_DEV) * 0.01
    sampling_locations = torch.rand(N, Lq, M, L, P, 2, device=_TEST_DEV)
    attention_weights = torch.rand(N, Lq, M, L, P, device=_TEST_DEV) + 1e-5
...
def check_gradient_numerical(channels=4, grad_value=True, grad_sampling_loc=True, grad_attn_weight=True):
    value = torch.rand(N, S, M, channels, device=_TEST_DEV) * 0.01
    sampling_locations = torch.rand(N, Lq, M, L, P, 2, device=_TEST_DEV)
    attention_weights = torch.rand(N, Lq, M, L, P, device=_TEST_DEV) + 1e-5
    attention_weights /= attention_weights.sum(-1, keepdim=True).sum(-2, keepdim=True)
    im2col_step = 2
