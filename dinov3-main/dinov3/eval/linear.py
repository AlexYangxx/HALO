    ...
    for n in n_last_blocks_list:
        for avgpool in [True]:
            for _lr in learning_rates:
                ...
                _lc_dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                linear_classifier = linear_classifier.to(_lc_dev)
...
def train_linear_classifiers(
    *,
    ...
    (linear_classifiers, start_iter, max_iter, criterion, optimizer, scheduler, best_accuracy,) = setup_linear_training(
        config=train_config,
        sample_output=feature_model(
            train_dataset[0][0].unsqueeze(0).to(next(feature_model.parameters()).device)
        ),
        training_num_classes=training_num_classes,
    ...
    header = "Training"
    _fm_dev = next(feature_model.parameters()).device
    for data, labels in metric_logger.log_every(
        ...
        data = data.to(_fm_dev, non_blocking=True)
        labels = labels.to(_fm_dev, non_blocking=True)
