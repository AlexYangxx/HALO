...
class SSLMetaArch(nn.Module):
    ...
    def forward_backward(
        ...
        global_crops = data["collated_global_crops"].to(_dev, non_blocking=True)
        local_crops = data["collated_local_crops"].to(_dev, non_blocking=True)
        masks = data["collated_masks"].to(_dev, non_blocking=True)
        mask_indices_list = data["mask_indices_list"].to(_dev, non_blocking=True)
        masks_weight = data["masks_weight"].to(_dev, non_blocking=True)
        n_masked_patches_tensor = data["n_masked_patches"].to(_dev, non_blocking=True)
        ...
        if self.has_gram_teacher:
            ...
            gram_teacher_crops = data["collated_gram_teacher_crops"].to(_dev, non_blocking=True)
        else:
